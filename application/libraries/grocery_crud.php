		...
		switch ($real_type) {
			...
			case 'upload_file':
				...
				else
				{
					...
					if($is_image)
					{
						$file_url_anchor .= ' class="image-thumbnail2"><img src="'.$file_url.'" width="50px">';
					}
					else
					{
...
class grocery_CRUD_Layout extends grocery_CRUD_Model_Driver
{
	...
	protected function get_upload_file_input($field_info, $value)
	{
		...
		$image_class = $is_image ? 'image-thumbnail2' : '';
		...
		$input .= $is_image ? " $image_class'><img src='".$file_url."' width='50px'>" : "' target='_blank'>$value";
		$input .= "</a> ";
